...
function executeCommand(command: string, errorMessage: string) {
  try {
    ...
    console.error(bold(red(`❌ ${errorMessage}`)))
    process.exit(1)
  }
}
...
function ensureDistIndexExists() {
  if (existsSync(DIST_INDEX_PATH)) {
    console.log(
      dim('  ') +
        green(`✓ dist/${DIST_INDEX_FILE} exists`) +
        dim(' (skip build)')
    )
    return
  }
  ...
  console.log(
    dim('  ') +
      yellow(`⚠️  dist/${DIST_INDEX_FILE} not found, running build...`)
  )
  executeCommand('pnpm build', 'Build failed')

  if (!existsSync(DIST_INDEX_PATH)) {
    console.error(
      bold(red(`\n  ❌ Build failed: dist/${DIST_INDEX_FILE} still missing`))
    )
    process.exit(1)
  }
  console.log(dim('  ') + green(`✓ Build completed`))
}
...
function extractConfiguration() {
  ...
  console.log(`${dim('  → ')}Extracting DEFAULT_CONFIG...`)
  ...
  console.log(`${dim('  → ')}Extracting PRESET_PLUGINS...`)
  const presetPlugins = extractCodeBlock(
    content,
    'const PRESET_PLUGINS',
    'object'
  )

  console.log(`${dim('  → ')}Extracting IGNORE_FILES...`)
  const ignoreFiles = extractCodeBlock(content, 'const IGNORE_FILES', 'object')

  console.log(`${dim('  → ')}Extracting king3 function...`)
  const king3Function = extractCodeBlock(content, 'const king3', 'function')

  console.log(dim('  ') + green('✓ All configurations extracted'))
...
function generatePrettierConfig(config: {
  ...
  console.log(dim('  ') + green('✓ Written to ') + cyan('prettier.config.js'))
}
...
function prettiergen() {
  try {
    console.log(bold(cyan('🚀 Starting prettier config generation...\n')))
    ...
    console.log(bold(green('\n🎉 Prettier config generated successfully!\n')))
  } catch (error) {
    console.error(
      `${
        bold(red('\n❌ Generation failed: ')) + red((error as Error).message)
      }\n`
    )
    process.exit(1)
  }
